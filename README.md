...
O projeto é organizado em camadas bem separadas, cada tecnologia com um papel claro.
...
| **Departamentos** | Técnicos são segmentados por departamento (por exemplo, HelpDesk, Dev, Infra); o HelpDesk faz a triagem inicial e tem visão ampla, os demais só veem os próprios chamados. Chamados podem ser encaminhados entre departamentos ou devolvidos ao HelpDesk. | Backend |
| **Ciclo de vida do chamado** | Abertura, atribuição/auto-atribuição de técnico, mudança de status, edição de conteúdo, reabertura, encaminhamento entre departamentos e fechamento pelo cliente. Cada ação gera um evento de histórico. | Frontend + Backend |
| **SLA** | Prazos de resposta/resolução calculados por prioridade, com um serviço em background (`SlaMonitorService`) que reavalia periodicamente a situação (em dia, em risco, vencido) e dispara notificações. Prazos podem ser ajustados manualmente com justificativa. | Backend |
...
| **Administração** | Gestão de usuários (criar, ativar/desativar, redefinir senha, vincular departamentos), departamentos e categorias diretamente pela interface. | Frontend + Backend |
...
**Modelagem de dados:** ver [`database/MODELO-ER.md`](database/MODELO-ER.md), com o diagrama ER e o dicionário de dados das entidades.

**Contrato da API:** ver [`docs/openapi.yaml`](docs/openapi.yaml), a especificação OpenAPI 3.1 (endpoints, payloads e códigos de status). Abre em qualquer Swagger UI / Redoc. Em desenvolvimento, o backend também expõe documentação interativa em `/swagger` (ver [`backend/README.md`](backend/README.md)).

**Wireframes:** ver [`docs/wireframes.html`](docs/wireframes.html), esboços de baixa fidelidade das telas principais. Abra no navegador.
...
> Ao acessar de outra máquina da rede (por exemplo, Docker rodando num host remoto), troque `localhost` pelo IP dessa máquina nas URLs abaixo; o `docker compose` não faz isso automaticamente.
...
- **Testes unitários** (`Unit/`): cálculo de situação de SLA, entre outros. Não têm dependências externas e rodam em qualquer ambiente.
...
O deploy de referência é **containerizado**: `docker-compose.yml` builda as imagens de `backend/Dockerfile` (multi-stage, SDK para runtime `aspnet:9.0-alpine`) e `frontend/Dockerfile` (multi-stage, Node para `nginx:alpine` servindo o build de produção do Angular), além de subir um container `postgres:16-alpine`. O `nginx` do frontend serve os arquivos estáticos e faz proxy reverso de `/api/*` (REST) e `/hubs/*` (SignalR, com upgrade de conexão para WebSocket) para o backend, ver [`frontend/nginx.conf`](frontend/nginx.conf). Todos os três serviços declaram `HEALTHCHECK`, e o backend só sobe depois que o `db` estiver saudável (`depends_on: condition: service_healthy`).
...
Projeto desenvolvido por **Gustavo Gomes Ferreira**, cobrindo o ciclo completo: interface, API, banco de dados e infraestrutura.

[GitHub](https://github.com/GuGomess) · [LinkedIn](https://www.linkedin.com/in/gustavogomesferreira/)
